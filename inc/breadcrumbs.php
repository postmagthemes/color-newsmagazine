...
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the color_newsmagazine_breadcrumb_trail class,
 * which should be used in theme templates.
 *
 * @since  0.1.0
 * @access public
 * @param  array $args Arguments to pass to color_newsmagazine_breadcrumb_trail.
 * @return void|string
 */
function color_newsmagazine_breadcrumb_trail( $args = array() ) {

	$breadcrumb = apply_filters( 'color_newsmagazine_breadcrumb_trail_object', null, $args );

	if ( !is_object( $breadcrumb ) )
		$breadcrumb = new Color_newsmagazine_Breadcrumb_Trail( $args );
 ...
 * @access public
 */
class Color_newsmagazine_Breadcrumb_Trail {
	...
	/**
	 * Sets up the breadcrumb trail properties.  Calls the `color_newsmagazine_breadcrumb_trail::add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see color_newsmagazine_breadcrumb_trail::set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see color_newsmagazine_breadcrumb_trail::set_post_taxonomy()
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'color_newsmagazine_breadcrumb_trail_args', wp_parse_args( $args, $defaults ) );
	...
	public function trail() {
		...
		$breadcrumb = apply_filters( 'color_newsmagazine_breadcrumb_trail', $breadcrumb, $this->args );
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:',                               'color-newsmagazine' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'color-newsmagazine' ),
			'home'                => esc_html__( 'Home',                                  'color-newsmagazine' ),
			'error_404'           => esc_html__( '404 Not Found',                         'color-newsmagazine' ),
			'archives'            => esc_html__( 'Archives',                              'color-newsmagazine' ),
			// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
			'search'              => esc_html__( 'Search results for &#8220;%s&#8221;',   'color-newsmagazine' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s',                               'color-newsmagazine' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s',                             'color-newsmagazine' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s',                               'color-newsmagazine' ),
		...
		$this->labels = apply_filters( 'color_newsmagazine_breadcrumb_trail_labels', wp_parse_args( $this->args['labels'], $defaults ) );
	}
	...
	protected function set_post_taxonomy() {
		...
		$this->post_taxonomy = apply_filters( 'color_newsmagazine_breadcrumb_trail_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
	}
	...
	protected function add_items() {
		...
		$this->items = array_unique( apply_filters( 'color_newsmagazine_breadcrumb_trail_items', $this->items, $this->args ) );
	}
				...
				if ( isset( $matches ) ) {
					...
					foreach ( $matches as $match ) {
						...
						if ( !empty( $post_types ) ) {
							...
							// Core filter hook.
							$label = apply_filters( 'color_newsmagazine_post_type_archive_title', $label, $post_type_object->name );
	...
	protected function add_post_type_archive_items() {
		...
		if ( is_paged() )
			$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_post_type_archive_link( $post_type_object->name ) ), color_newsmagazine_post_type_archive_title( '', false ) );

		elseif ( true === $this->args['show_title'] )
			$this->items[] = color_newsmagazine_post_type_archive_title( '', false );
	}
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'color-newsmagazine' ) ) );
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'color-newsmagazine' ) ) );
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'color-newsmagazine' ) ) );
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'color-newsmagazine' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'color-newsmagazine' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'color-newsmagazine' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'color-newsmagazine' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'color-newsmagazine' ) ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'color-newsmagazine' ) ) ) );

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'color-newsmagazine' ) ) ) );

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'color-newsmagazine' ) ) ) );
