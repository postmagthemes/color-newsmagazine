...
if( ! class_exists( 'Color_newsmagazine_Control_Repeater' ) ) {
    /**
     * Repeater control
     */
    class Color_newsmagazine_Control_Repeater extends WP_Customize_Control {
        ...
        public function __construct( $manager, $id, $args = array() ) {
            ...
            $this->row_label = array(
                'type' => 'text',
                'value' => esc_attr__( 'row', 'color-newsmagazine' ),
                'field' => false,
            );
            ...
            if ( empty( $this->button_label ) ) {
             ...
             $this->button_label = sprintf( esc_attr__( 'Add new %s', 'color-newsmagazine' ), $this->row_label['value'] );
         }
                ...
                foreach ( $this->fields as $field ) {
                    if ( isset( $field['type'] ) ){
                        if( 'color' === $field['type'] ){ 
                            ...
                            wp_enqueue_style( 'font-awesome', get_template_directory_uri(). '/assets/css/font-awesome-5.css', array(), '5.0' );
                        }
                    }
                }
            }
            wp_enqueue_script( 'color-newsmagazine', get_template_directory_uri() . '/assets/customizer/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
            wp_enqueue_style( 'color-newsmagazine', get_template_directory_uri() . '/assets/customizer/repeater.css', null );
        }
        ...
        protected function render_content() {
            ...
            <?php if ( isset( $this->choices['limit'] ) ) : ?>
                <p class="limit"><?php /* translators: %s: Limit Number. */
                printf( esc_attr__( 'Limit: %s rows', 'color-newsmagazine' ), esc_html( $this->choices['limit'] ) ); ?></p>
            <?php endif; ?>
        ...
        public function repeater_js_template() {
            ...
            <label>
                ...
                <span class="description customize-control-description">{{{ field.description }}}</span>
                <# } #>
                <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'color-newsmagazine' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
            ...
            <figure class="kirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'color-newsmagazine' ); ?>" >
                <# if ( field.default ) { #>
                <# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
                <img src="{{{ defaultImageURL }}}">
                <# } else { #>
                <?php esc_attr_e( 'No Image Selected', 'color-newsmagazine' ); ?>
                <# } #>
            </figure>
            
            <div class="actions">
                <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php  esc_html_e( 'Remove', 'color-newsmagazine' ); ?></button>
                <button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'color-newsmagazine' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'color-newsmagazine' ); ?>" >
                    <# if ( field.default ) { #>
                    <?php esc_attr_e( 'Change Image', 'color-newsmagazine' ); ?>
                    <# } else { #>
                    <?php esc_attr_e( 'Add Image', 'color-newsmagazine' ); ?>
                    <# } #>
                </button>
                <# if ( field.default.id ) { #>
            ...
            <figure class="kirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'color-newsmagazine' ); ?>" >
                <# if ( field.default ) { #>
                ...
                <# } else { #>
                <?php esc_attr_e( 'No File Selected', 'color-newsmagazine' ); ?>
                <# } #>
            </figure>
            
            <div class="actions">
                <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
                <button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'color-newsmagazine' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'color-newsmagazine' ); ?>" >
                    <# if ( field.default ) { #>
                    <?php esc_attr_e( 'Change File', 'color-newsmagazine' ); ?>
                    <# } else { #>
                    <?php esc_attr_e( 'Add File', 'color-newsmagazine' ); ?>
                    <# } #>
                </button>
                <# if ( field.default.id ) { #>
                <input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
        ...
        <# }); #>
        <button type="button" class="button-link repeater-row-remove"><?php  esc_html_e( 'Remove', 'color-newsmagazine' ); ?></button>
    </div>
</li>
</script>
<?php
        ...
        function get_icon_list(){
            ...
            $icons5 = include wp_normalize_path( get_template_directory() . '/inc/fontawesome5.php' ); 
            if( $icons5 ){ ?>
                <div class="font-awesome-list-template">
                    <div class="font-awesome-list">
                        <ul class="font-group">
                            ...
                            <?php
                            foreach( $icons5 as $font5 => $fontval ){
                                echo '<li><i class="' . esc_attr( $font5 ) . '"></i></li>';
                            }
                            ?>
                        </ul>
                    </div>
